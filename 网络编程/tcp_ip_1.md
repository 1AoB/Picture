...
int main(int argc,char *argv[])
{
	...
	while(1)
	{
		...
		puts("return epoll_wait");//////////如果程序的运行结果中:多次调用这句代码,说明就是条件触发
		for(i=0;i<event_cnt;i++)
		{
			if(ep_events[i].data.fd==serv_sock)
			{
...
答:select模型是以条件触发的方式工作的,如果输入缓冲中还剩有数据,肯定会注册事件.
```
...
为了在发送错误时提供额外的信息,LInux声明了如下全局变量:int errno;

为了访问该变量,我们需要引入error.h头文件,因为此头文件中有上述变量的extern声明.

每个错误对应的errno的值都不同,没必要记住所有可能的值.但是要记得read函数:
"read函数发现输入缓冲中没有数据可读时返回-1,同时在error中保存EAGAIN常量"
...
cmd:表示函数调用的目的。
如果向第二个参数cmd传递F_GETFL，可以获得第一个参数所指的文件描述符属性(int型)。反之，如果传递F_SETFL，可以更改文件描述符属性。若希望将文件（套接字)改为非阻塞模式，需要如下2条语句:
...
提问:为何需要通过errno确认错误原因?
答:"边缘触发中,接收数据时仅注册1次该事件".正因为这个特点,一旦发送输入相关事件,就应该读取输入缓冲中的全部数据.因此需要验证缓冲是否为空.
"read函数返回-1,变量errno中的值为EAGAIN时,说明没有数据可读"

既然如此,为何还需要将套接字变成非阻塞模式?在边缘触发方式下,以阻塞方式工作的read & write函数可能引起服务器端的长时间停顿."所以,边缘触发方式中一定要采用非阻塞read&write函数"
...
int main(int argc,char *argv[])
{
	...
	epfd=epoll_create(EPOLL_SIZE);//例程,用到EPOLL_SIZE
	ep_events=malloc(sizeof(struct epoll_event)*EPOLL_SIZE);//用到EPOLL_SIZE
	
	setnonblockingmode(serv_sock);
	event.events=EPOLLIN;
	event.data.fd=serv_sock;
	event_ctl(epfd,EPOLL_CTL_ADD,sev_sock,&event);
	
	while(1)
	{
		event_cnt = epoll_wait(epfd,ep_events,EPOLL_SIZE,-1);//用到EPOLL_SIZE
		if(event_cnt == -1)
		{
			puts("epoll_wait() error");
			break;
		...
		for(i=0;i<event_cnt;i++)//遍历
		{
			//如果是服务的文件描述符,就说明有新客户端进行连接;
			//如果是其他的客户端的文件描述符,就说明客户端来数据,我们就要read了
			if(ep_events[i].data.fd==serv_sock)
			{
				adr_sz=sizeof(clnt_adr);
				clnt_sock=accept(serv_sock,(struct sockaddr*)&clnt_adr,&adr_sz);
				setnonblockingmode(clnt_sock);//让新连接的客户端非阻塞
				event.events=EPOLLIN|EPOLLET;//可读并且设置边缘触发
				event.data.fd=clnt_sock;
				epoll_ctl(epfd,EPOLL_CTL_ADD,clnt_sock,&event);
				printf("connected client:%d \n",clnt_sock);
			}
			else
			{
				while(1)
				{
					...
					{
						if(errno==EAGAIN)//read函数返回-1且errno值为EAGAIN时,意味着读取了输入缓冲中的全部数据,因此需要通过break语句跳出循环
						{
							break;
						}
					}
...
void setnonblockingmode(int fd)//设置文件描述符非阻塞
{
...
边缘触发可以做到如下这点:
"可以分离接收数据和处理数据的时间点!"
因此，即使输人缓冲收到数据（注册相应事件)，服务器端也能决定读取和处理这些数据的时间点，这样就给服务器端的实现带来巨大的灵活性。


提问:"边缘触发是否更快?能快多少呢?"
答:从实现模型的角度看，边缘触发更有可能带来高性能，但不能简单地认为“只要使用边缘触发就一定能提高速度”。
...
(5) epoll以条件触发或边缘触发方式工作。二者有何区别?从输人缓冲的角度说明这2种方式通知事件的时间点差异。
...
```cpp
上下文切换的概念:
运行程序前需要将相应进程信息读人内存，如果运行进程A后需要紧接着运行进程B，就应该将进程A相关信息移出内存，并读入进程B相关信息。这就是上下文切换。但此时进程A的数据将被移动到硬盘，所以上下文切换需要很长时间。即使通过优化加快速度，也会存在一定的局限。


为了将进程的各种劣势降至最低限度（不是直接消除）而设计的一种“轻量级进程”,即线程。线程相比于进程具有如下优点:
...
POSIX是Portable Operating System Interface for Computer Environment(适用于计算机环境的可移植操作系统接口)的简写，是为了提高UNIX系列操作系统间的移植性而制定的API规范。下面介绍的线程创建方法也是以POSIX标准为依据的。因此，它不仅适用于Linux，也适用于大部分UNIX系列的操作系统。
...
->成功时返回0,失败时返回其他值
	thread:保存新创建线程ID的变量地址值.线程与进程"相同",也需要用于区分不同线程的ID.
	attr:用于传递线程属性的参数,传递NULL时,创建默认属性的线程
	start_routine:相当于线程main函数的,在单独执行流中执行的函数地址值(函数指针)
	arg:通过第三个参数传递调用函数时包含传递参数信息的变量地址值

(注 : 在C语言中，restrict关键字用于修饰指针(C99标准)。通过加上restrict关键字，编程者可提示编译器：在该指针的生命周期内，其指向的对象不会被别的指针所引用。)
...
(简而言之,调用该函数的进程(或线程)将进入等待状态,直到第一个参数为ID的线程终止为止.而且可以得到线程的main函数返回值,所以该函数比较有用.)
...
int main(int argc,char *argv[])
{
	...
	if(pthread_join(t_id,&thr_ret) != 0)//通过thr_ret来接收t_id线程的返回值
	{
...
```
## 可在临界区内调用的函数(-D_REENTRANT选项的使用!!!)(P292)
```cpp
可在临界区内调用的函数(-D_REENTRANT选项的使用!!!)
...
"线程安全函数的名称后缀通常为_r (这与Windows平台不同)。"既然如此，多个线程同时访问的代码块中应该调用gethostbyname_r，而不是gethostbyname?
当然!但这种方法会给程序员带来沉重的负担。幸好可以通过如下方法自动将gethostbyname函数调用改为gethostbyname_r函数调用:
"声明头文件前定义_REENTRANT宏。"
...
"总结一下: -D_REENTRANT选项 可以让 系统 自动 将非线程安全的函数 转化为 具有相同功能的、且线程安全的函数。"
...
上面都是创建1个线程的示例,下面给出1个创建多个线程的示例:

将要介绍的示例将计算1到10的和,但并不是在main函数中进行累加运算,而是创建2个线程,其中一个线程计算1到5的和，另一个线程计算6到10的和，main函数只负责输出运算结果。
这种方式的编程模型称为"工作线程（Worker thread）模型"。计算1到5之和的线程与计算6到10之和的线程将成为main线程管理的工作 ( Worker )。

//thread3.c
#include<stdio.h>
#include<pthread.h>
int sum = 0;
void * thread_summation(void *arg)//线程函数
{
	int start = ((int*)arg)[0];
	int end = ((int*)arg)[1];
...
以上结果虽然正确,但是示例本身存在问题.此处存在临界区的相关问题
*/

下面在来一个例子:增加了发生临界区相关错误的可能性:
...
long long num = 0;//long long 是64为整数
...
int main(int argc,char *argv[])
{
	pthread_t thread_id[NUM_THREAD];//thread
	int i;
	...
	for(i=0;i<NUM_THREAD;i++)//回收100个线程
		pthread_join(thread_id[i],NULL);
		
	printf("result: %lld\n",num);
	return 0;
}
上述示例中共创建了100个线程,其中一半执行threa_inc函数中的代码,另一半则执行thread_des函数中的代码.全局num经过增减过程应该为0
/*
运行结果: thread4.c
root@my_linux:/tcpip# gcc thread4.c -D_REENTRANT -o th4 -1pthreadroot@my_linux : / tcpip# ./th4
sizeof long long: 8
result: 4284144869
...
上述代码(thread4.c)中,最终的结果是意向不到的.究其原因:没有进行同步.
当前线程访问变量num时应该阻止其他线程访问,直到该线程完成运行,这就是同步(Synchronization).
```
...
->成功时返回0,失败时返回其他值
	...
	- mutex:创建互斥量时传递保存互斥量变量的地址值，销毁时传递需要销毁的互斥量变量的地址值。
...
//上述示例中，thread_des函数比thread_inc函数多调用49,999,999次互斥量lock、unlock函数，表现出人可以感知的速度差异。
```
```cpp
信号量
...
->成功时返回0,失败时返回其他值
	...
	- pshared:传递其他值时,创建可由多个进程共享的信号量;传递0时,创建只允许1个进程内部使用的信号量.我们需要完成同一进程内的薪酬同步,故传递0.
	- value:指定新创建的信号量初始值.下面介绍信号量中相当于互斥量lock、unlock的函数:
...
注:信号量的值不能小于0,因此,在信号量为0的情况下调用wem_wait函数时,调用函数的线程将进入阻塞状态(因为函数未返回).
当然,此时如果有其他线程调用sem_post函数,信号量的值将变为1,而原本阻塞的线程可以将信号量重新减为0并跳出阻塞状态.
